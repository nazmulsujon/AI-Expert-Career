...
const { language } = useContext(MyContext)
  ...
  return (
    <div className="mt-[80px] mb-[35px] py-[14px]">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5 mt-[100px] lg:mt-[30px] ">
        {aboutCard
          .filter((card) => active === 'all' || card.category === active)
          .map(({ id, img, headline, info, price }) => (
            <div
              key={id}
              className="px-[17px] border-[0.5px] border-gray-50 shadow-lg hover:shadow-lg relative bg-[#FFFFFF] rounded-[10px] py-[14px] md:h-[400px] h-[430px]">
              <center>
                <img
              ...
              <div className="">
                <h3 className="text-lg font-bold mb-[8px]">
                  {headline.slice(0, 40)}...
                </h3>
                <h5 className="mb-[20px] text-sm">{info.slice(0, 100)}</h5>
                <div className="flex absolute bottom-[13px] w-[89%] m-auto left-0 right-0 justify-between items-center ">
                  <Link
                    className="px-[32px] duration-200 my-btn py-[8px] bg-[#FF265A] rounded-md text-white h-full shadow-lg"
                    to={`/payment/${id}`}
                  >
